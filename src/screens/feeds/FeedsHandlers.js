...
export const handleLikePress = async (
  ...
  const feedIndex = updatedFeedsLikes.findIndex(
...
const updateFeedLikes = (
  ...
  const updatedFeedsLikes = [...allFeedsCollection];
  const feed = updatedFeedsLikes[feedIndex];
  feed.likes = newLikes;
  feed.liked = liked;
...
const revertFeedLikes = (
  ...
  const feed = revertedFeedsLikes[feedIndex];
...
export const handleLikeComment = async (
  ...
  const selectedFeedArray = updatedFeedsCollection; // Get the array of feeds at currentHorizontalIndex

  // Find the specific feed item
  const feedIndex = updatedFeedsCollection.findIndex(
...
const updateCommentLikes = (
  ...
  const updatedFeedsCollection = [...allFeedsCollection];
  updatedFeedsCollection[feedIndex].comments[
    commentIndex
  ].likes = newLikes;
  updatedFeedsCollection[feedIndex].comments[
    commentIndex
...
const revertCommentLikes = (
  ...
  const comment =
    revertedCommentLikes[feedIndex].comments[commentIndex];
...
export const handleAddComment = async (
  ...
  const updatedFeedsCollection = [...allFeedsCollection];
  const selectedFeedArray = updatedFeedsCollection;
  ...
  if (!internetState) {
    updatedFeedsCollection[feedIndex].comments =
      updatedFeedsCollection[feedIndex].comments.filter(
    ...
    if (videoIndex !== -1) {
      ...
      if (commentIndex !== -1) {
        ...
        const likedReplyIndex = likedVideos[videoIndex].likedComments[
          ...
          (likedReply) => likedReply.replyIndex === replyIndex,
        );
...
const updateReplyLikes = (
  ...
  const reply =
    updatedReplyLikes[feedIndex].comments[commentIndex]
      .reply[replyIndex];
...
const revertReplyLikes = (
  ...
  const reply =
    updatedRevertedLikes[feedIndex].comments[
      commentIndex
    ].reply[replyIndex];
